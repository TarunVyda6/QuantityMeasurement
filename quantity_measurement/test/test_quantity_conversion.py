...
def test_for_length_conversion(first_type, first_value, second_type, second_value, result):
    assert QuantityMeasurement(first_type, first_value).compare_with(
        QuantityMeasurement(second_type, second_value)) == result
...
def test_for_adding_two_lengths(first_type, first_value, second_type, second_value, result):
    assert QuantityMeasurement(first_type, first_value) + QuantityMeasurement(second_type, second_value) == result
...
@pytest.mark.parametrize('first_type, first_value, second_type,second_value,result',
                         [(Temperature.fahrenheit, 89.6, Weights.gram, 32, TypeMismatchException)])
def test_for_temperature_addition(first_type, first_value, second_type, second_value, result):
    with pytest.raises(result):
        assert QuantityMeasurement(first_type, first_value) + QuantityMeasurement(second_type, second_value)
        assert QuantityMeasurement(first_type, first_value).compare_with(QuantityMeasurement(second_type, second_value))
